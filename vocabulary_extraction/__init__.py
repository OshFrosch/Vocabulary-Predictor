...
from vocabulary_extraction.utils.output_utils.add_infos_to_df import add_infos_to_df
from vocabulary_extraction.utils.output_utils.create_csv_from_df import (
    create_csv_from_df,
)
from vocabulary_extraction.utils.output_utils.create_df_from_doc import (
    create_df_from_doc,
)
from vocabulary_extraction.utils.output_utils.predict_output_size import (
    predict_outputsize,
)
...
def main():
    ...
    input_type_args = input_args.add_mutually_exclusive_group(required=True)
    ...
    output_args.add_argument(
        "--csv_separator",
        metavar="SEP",
        help="Define a custom CSV separator (default: , )",
    )
    ...
    if args.raw:
        ...
        logger.info("Processing html input")
    ...
    if args.to_csv:
        create_csv_from_df(df_final, args.to_csv, sep=args.csv_separator)
    else:
        create_csv_from_df(df_final, sys.stdout, sep=args.csv_separator)
